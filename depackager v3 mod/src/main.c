...
void install_pkg(const char *file)
{
	...
	for (i = 0; i < pspcount; i++) {
		...
		if (is_file[i]) {
			pspDebugScreenClear(); 
			printScreenCenter(0, "PSP depackager v3", 0xFFFF00);
			printScreenCenter(1, "PSP PKG installer", 0xFF0000);
			...
			char demostring [100];
			int ii;
			pspDebugScreenPutChar(34,104,0xFFFFFF, '[');
			pspDebugScreenPutChar(446,104,0xFFFFFF, ']');
			for (ii=40; ii < 440 ;ii+=4) {
			pspDebugScreenPutChar(ii,104,0x4444FF, '>');
			...
			for (j = 0; j < (int)(file_size[i] >> 4); j++) {
				if (szcheck == 1024 * 1024) {
					...
					//strncat(demostring, '%', 1);
					//printScreenCenter(20, demostring, 0xFFFFFF);
					
					percent = (percent / totaal);
					percent = (percent *4 );
					percent = (percent + 40);
					
					//sprintf(demostring, "%i", percent);
					pspDebugScreenPutChar(percent,144,0x44FF44, '>');
					pspDebugScreenPutChar(34,144,0xFFFFFF, '[');
					pspDebugScreenPutChar(446,144,0xFFFFFF, ']');
				}
	...
	pspDebugScreenClear();  
	pspDebugScreenInit();
	printScreenCenter(16, "GAME INSTALLED", 0xFFFFFF);
